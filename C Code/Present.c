...
char* decrypt(char* ciphertext, char* key){
    ...
    //byte* temp = malloc (8 * sizeof(byte));
    ...
    for (i=0; i<31; i++){
        ...
        printf("\nROUND %d \n",i);
        char* temp_str = fromLongToHexString(state);
        printf("STATE:\t \t %s \n",temp_str);
        //byte* temp_state = fromLongToBytes(state);

        //printf("STATE:/t/t");
        //for (int i=0;i<8;i++)
        //    printf("%u02x ", temp_state[i].nibble1<<4|temp_state[i].nibble2 );
        //printf("\n");
        char* temp_str_key = fromLongToHexString(subkeys[31 - i]);
        printf("RKEY:\t \t %s \n",temp_str_key);

        state = state ^ subkeys[31 - i];

        char* temp_str_addrkey = fromLongToHexString(state);
        printf("AD_RKEY:\t %s \n",temp_str_addrkey);

        
       //byte* temp = fromLongToBytes(state);
       //for (int i=0;i<8;i++)
       //printf("%u02x ", temp[i].nibble1<<4|temp[i].nibble2 );
       //printf("\n");
        //perform the inverse permutation defined above
        state = inversepermute(state);

        char* temp_str_IP = fromLongToHexString(state);
        printf("IP:\t\t %s \n",temp_str_IP);
        ...
        char* temp_str_IS = fromLongToHexString(state);
        printf("IS:\t\t %s \n",temp_str_IS);
        //free the memory of the byte array (not needed anymore)
        free(stateBytes);
    }
